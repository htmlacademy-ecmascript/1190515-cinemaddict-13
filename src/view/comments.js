...
const createCommentTemplates = (comments) => {
  const setDateView = (date) => new Date().setMonth(new Date().getMonth() - 1) > date ? dayjs(date).format(`DD MM YYYY`) : dayjs(date).toDate();
...
export default class CommentsView extends AbstractComponent {
